...
class TestAppFactory:
    ...
    def test_exception_handler(self, app):
        """Test generic exception handler (500 error)"""
        # We define a temporary route that will raise an exception
        @app.route('/test-error')
        def trigger_error():
            raise Exception('This is a test error')
        
        # We must temporarily disable TESTING mode. 
        # In TESTING mode, Flask re-raises exceptions instead of handling them with the app's error handler.
        ...
        # Re-enable TESTING mode for other tests
...
class TestIntegration:
    ...
    def test_multiple_todos_workflow(self, client):
        ...
        # Update one
        todo_id_to_update = data['data'][2]['id'] # Update the third todo
        response = client.put(f'/api/todos/{todo_id_to_update}', json={'completed': True})
        assert response.status_code == 200
        
        # Delete one
        todo_id_to_delete = data['data'][4]['id'] # Delete the fifth todo
        response = client.delete(f'/api/todos/{todo_id_to_delete}')
        assert response.status_code == 200
